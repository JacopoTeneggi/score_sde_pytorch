...
def get_step_fn(
    ...
    def step_fn(state, batch):
        ...
        """
        model_engine = state["model_engine"]
        if train:
            loss = loss_fn(model_engine, batch)
            model_engine.backward(loss)
            model_engine.step()
            state["ema"].update(model_engine.parameters())
        else:
            with torch.no_grad():
                ema = state["ema"]
                ema.store(model_engine.parameters())
                ema.copy_to(model_engine.parameters())
                loss = loss_fn(model_engine, batch)
                ema.restore(model_engine.parameters())
