...
def get_dataset(config):
    if config.data.dataset == "CIFAR10":
        preprocess_transform = t.Compose(
            [t.ToTensor(), t.Resize(config.data.image_size)]
        )
        root = os.path.join("data", "CIFAR10")
        train_dataset = CIFAR10(root, train=True, transform=preprocess_transform)
        val_dataset = CIFAR10(root, train=False, transform=preprocess_transform)
        return train_dataset, val_dataset
    elif config.data.dataset == "CelebAMaskHQ":
        preprocess_transform = t.Compose(
            [t.ToTensor(), t.Resize(config.data.image_size)]
        )
        root = os.path.join("data", "CelebAMask-HQ")
        train_dataset = CelebAMaskHQ(root, "train", preprocess_transform)
        val_dataset = CelebAMaskHQ(root, "val", preprocess_transform)
        return train_dataset, val_dataset
    else:
...
#         train_split_name, eval_split_name = "train", "val"
...
#         if config.data.dataset == "CelebAMaskHQ":
#             dataset_builder = tf.data.TFRecordDataset(
#                 os.path.join(config.data.tfrecords_path, f"{split}-r08.tfrecords")
#             )
